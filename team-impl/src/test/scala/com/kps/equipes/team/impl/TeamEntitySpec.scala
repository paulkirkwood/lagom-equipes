...
class TeamEntitySpec extends WordSpec with Matchers with BeforeAndAfterAll {
  ...
  "The team entity" should {
    ...
    "create a team" in withTestDriver { driver =>
      val teamSky = Team(UUID.randomUUID(), "Team Sky", true, UUID.randomUUID())
      val outcome = driver.run(CreateTeam(teamSky))
    ...
    "get team" in withTestDriver {driver =>
      val carrera = Team(UUID.randomUUID(), "Carrera", true, UUID.randomUUID())
    ...
    "updating a team name" in withTestDriver { driver =>
      val sem = Team(UUID.randomUUID(), "Sem-France Loire", true, UUID.randomUUID())
      driver.run(CreateTeam(sem))
      val outcome = driver.run(ChangeTeamName("Skil-Mavic"))
      outcome.replies should contain only Done
      outcome.events(0) should matchPattern { case TeamNameChanged(Team(sem.id, "Skil-Mavic", sem.active, sem.countryId)) => }
    }
    ...
    "allow changing a team's status" in withTestDriver { driver =>
      val kas = Team(UUID.randomUUID(), "KAS", true, UUID.randomUUID())
      driver.run(CreateTeam(kas))
      val outcome = driver.run(ChangeTeamStatus(false))
      outcome.replies should contain only Done
      outcome.events(0) should matchPattern { case TeamStatusChanged(Team(kas.id, kas.name, false, kas.countryId)) => }
    }
  }
}
